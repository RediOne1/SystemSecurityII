 ...
 * date:    24.01.2017
 */
public class VerifierLoadSignerParams implements State {

	private SchnorrRingSignatureWorldVerifier verifier;

	public VerifierLoadSignerParams(SchnorrRingSignatureWorldVerifier verifier) {
		this.verifier = verifier;
	}

	@Override
	public void onPrepare(INewMainView view) {
		view.printOutputMessage(new Message("Enter Signer Params JSON & tap SEND"));
	}
	...
	public void processInput(String input, IOnCompletionListener listener) {
		...
		Observable.just(input)
				...
				.doOnNext(response -> verifier.setSignerParams(response))
				.subscribe(
						response -> {
						},
	...
	public State getNextState() {
		return new VerifierVerify(verifier);
	}
}
